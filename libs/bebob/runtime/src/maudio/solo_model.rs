...
impl LevelCtlOperation<SoloPhysInputProtocol> for PhysInputCtl {
...
impl LrBalanceCtlOperation<SoloPhysInputProtocol> for PhysInputCtl {
...
impl LevelCtlOperation<SoloStreamInputProtocol> for StreamInputCtl {
...
impl SelectorCtlOperation<SoloSpdifOutputProtocol> for SpdifOutputCtl {
    const SELECTOR_NAME: &'static str = "S/PDIF-output-source";
    const SELECTOR_LABELS: &'static [&'static str] = &["S/PDIF-output-1/2"];
    const VAL_LABELS: &'static [&'static str] = &["stream-input-3/4", "mixer-output-3/4"];
}
