...
impl CtlModel<SndDice> for ExtensionModel {
    fn load(&mut self, unit: &mut SndDice, card_cntr: &mut CardCntr) -> Result<(), Error> {
        ...
        self.tcd22xx_ctl.load(unit, &mut self.req, &self.extension_sections, &caps, &src_labels, TIMEOUT_MS,
                              card_cntr)?;
        self.tcd22xx_ctl.cache(unit, &mut self.req, &self.sections, &self.extension_sections,
                               TIMEOUT_MS)?;
